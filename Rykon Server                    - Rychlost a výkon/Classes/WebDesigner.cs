...
namespace RykonServer
{
    class WebDesigner
    {
        ...
        */
        ...
        {
            return "<center><h2>not authorized</h2> <hr /><h3> Sorry You are not allowed </h3><form method='GET' action='exec.rk' > enter pass here <hr /> <input type='text' name='pass' /> <input type='hidden' name='CSRF' value='" + CSRF + "' /><input type='submit' /></form> </center>";
        }

        internal static string ControlCommandListindex(string p1, string p2, string p3, string CSRF, string password)
        {
            string doc = "<a href='exec?ps=$passwd$&CSRF=$CSRFtok$&com=msgbx&mbtitle=hello+It' >";
        ...
        public static string invalidAuthTok = "invalid auth token";
    }
}
